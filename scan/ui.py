...
def setGlobCompleter():
    ...
    def complete(t, s): return [x for x in glob.glob(t+"*")][s] if t else None
...
def loadConfig(confDir: str):
    ...
    try:
        ...
        utils.log_exc("ui.loadConfig:")
        print("Error in config file - cache not found: %s" %
              e, file=sys.stderr)
    else:
        del MENU_ENTRIES[0]
        ...
        MENU_ENTRIES.append(
            ("View usage of a Span broken down by host", spanUsageByHost))
        MENU_ENTRIES.append(
            ("Dump cache usage stats to file (Tabular YAML format)", dumpUsageToFile))
        print(CLEAR)
...
def getConfig():
    ...
    if choice.lower() != 'q':
        try:
            ...
            MENU_ENTRIES.append(("List Stripes in a Span", listSpanStripes))
            MENU_ENTRIES.append(
                ("View URLs of objects in a Span", listSpanURLs))
            MENU_ENTRIES.append(
                ("View usage of a Span broken down by host", spanUsageByHost))
            MENU_ENTRIES.append(
                ("Dump cache usage stats to file (Tabular YAML format)", dumpUsageToFile))
...
def searchSetting():
    ...
    while True:
        ...
        choice = input(
            "(name or regex of setting, 'l' to list settings, or 'q' to go back): ")
    ...
    while True:
        ...
        if os.path.exists(choice):
            ...
            try:
                ...
                print("Could not write to '%s'! (%s)" %
                      (choice, e), file=sys.stderr)
            except ValueError as e:
                utils.log_exc("ui.dumpUsageToFile:")
...
def nonInteractiveDump():
    ...
    clear = buffer.clear
    def output(x): return print(*x, sep='\n')
    bs = byteSized
...
def breakDownDump(spanFile: str = None):
    ...
    if spanFile:
        ...
        if spanFile not in spans:
            print("Error: '%s' is not a cache span!" %
                  (spanFile,), file=sys.stderr)
            return 1
        print("%TYAML 1.1\n---")
